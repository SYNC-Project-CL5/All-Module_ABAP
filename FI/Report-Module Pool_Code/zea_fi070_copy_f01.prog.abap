...
*    PERFORM FILL_SCREEN_0200 CHANGING CV_BN.   "AMOUNT SUM값 넣기
  ...
  ELSEIF PO_SENDER EQ GO_ALV_GRID_2.
    ...
    READ TABLE GT_DATA2 INTO GS_DATA2 INDEX LS_ROWS-INDEX.
*         COLLECT GS_DATA2 INTO GT_DATA3.
    PERFORM FILL_SCREEN_BOX_0200.   "FILL GT_DATA3
...
*--- READ TABLE - 선택한 Row =>> GT_DATA3에 옮기기
  LOOP AT LT_ROWS INTO LS_ROWS.
    READ TABLE GT_DATA2 INTO GS_DATA2 INDEX LS_ROWS-INDEX.

    IF GS_DATA2-AUGDT NE 'X' OR GS_DATA2-STATUS NE '3'.

      MOVE-CORRESPONDING GS_DATA2 TO GS_DATA3.
...
*  해당 전표가 이미 추가되어 있는지를 점검
  ...
  IF SY-SUBRC EQ 0.   "성공 = 이미 추가됨을 의미.
    ...
    MESSAGE '이미 미지급 리스트에 추가된 건입니다.' TYPE 'S'.
...
*          LS_DATA LIKE ZEA_BKNA.
  ...
  DATA : BEGIN OF LS_DATA,
           BANKCODE LIKE ZEA_BKNA-BANKCODE,
           BPBANK   LIKE ZEA_BKNA-BPBANK,
           ACCNT    LIKE ZEA_BKNA-ACCNT,
           SAKNR    LIKE ZEA_BKNA-SAKNR,
         END OF LS_DATA.
  ...
  SELECT * FROM ZEA_BKNA INTO CORRESPONDING FIELDS OF TABLE LT_DATA
    WHERE SAKNR NE ' '
    ORDER BY BANKCODE.
...
*&---------------------------------------------------------------------*
FORM FILL_SCREEN_0200 USING P_LT_ROWS TYPE LVC_T_ROW.

*--- Screen 200에 값 채우기
*     --  합계금액 채우기
  DATA : LV_AM1 TYPE ZEA_BSEG-DMBTR,
         LV_AM2 TYPE ZEA_BSEG-DMBTR.

  LOOP AT GT_DATA3 INTO GS_DATA3.

    IF GS_DATA3-D_WAERS EQ 'USD'.
      LV_AM1 = LV_AM1 + GS_DATA3-DMBTR.
      ZEA_BSEG-D_WAERS = GS_DATA3-D_WAERS.
    ENDIF.
    LV_AM2  = LV_AM2 + GS_DATA3-WRBTR.
    ZEA_BSEG-W_WAERS = GS_DATA3-W_WAERS.
    ZEA_BSEG-D_WAERS = 'USD'.
  ENDLOOP.

  ZEA_BSEG-DMBTR = LV_AM1 .
  ZEA_BSEG-WRBTR = LV_AM2 .
...
*--- Select ZEA_TCURR
  ...
  SELECT SINGLE * FROM ZEA_TCURR INTO  @GS_TCURR
    WHERE GDATU EQ @GS_DATA-BLDAT
    AND FCURR EQ 'KRW'
    AND TCURR EQ 'USD'.
  POST_RAT    = GS_TCURR-UKURS.

  IF POST_RAT EQ 0. "근데 읽어온 환율이 0이라면,
    ...
    POST_RAT = GS_TCURR-UKURS. "환율 보관 변수에 보관

  ENDIF.

* -- icon 설정
  IF  POST_RAT < ZEA_TCURR-UKURS . " A/P 발생일자 환율 < A/P 반제일자 환율
    " 1달러-1300원     > 1달러-1400원이 됨 .
...
*       TEXT                  = '외환차익 예상'
  ...
  IF POST_RAT EQ ZEA_TCURR-UKURS.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form FILL_SCREEN_BOX_0200
*&---------------------------------------------------------------------*
FORM FILL_SCREEN_BOX_0200 .

*--- Due Date Setting / Setting Screen 0100
  PERFORM SET_DUE_DATE.
  BLDAT = ZEA_BKPF-BLDAT.
  BUDAT = ZEA_BKPF-BUDAT.
  XBLNR = ZEA_BKPF-XBLNR.
  GJAHR = 2024.

ENDFORM.
*&---------------------------------------------------------------------*
*& Form CREATE_OBJECT3_0200
*&---------------------------------------------------------------------*
FORM CREATE_OBJECT3_0200 .
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form SET_ALV_FIELDCAT3_0200
*&---------------------------------------------------------------------*
FORM SET_ALV_FIELDCAT3_0200 .

  PERFORM GET_FIELDCAT_0200     USING    GT_DATA3
                                CHANGING GT_FIELDCAT3.

*-- Field Catalog 수정
  PERFORM MAKE_FIELD_CATALOG3.


ENDFORM.
*&---------------------------------------------------------------------*
*& Form GET_FIELDCAT_0200
*&---------------------------------------------------------------------*
FORM GET_FIELDCAT_0200 USING   PT_TAB TYPE STANDARD TABLE
                       CHANGING PT_FCAT TYPE LVC_T_FCAT.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form SET_ALV_EVENT3_0200
*&---------------------------------------------------------------------*
FORM SET_ALV_EVENT3_0200 .

ENDFORM.
*&---------------------------------------------------------------------*
*& Form DISPLAY_ALV3_0200
*&---------------------------------------------------------------------*
FORM DISPLAY_ALV3_0200 .
...
FORM MAKE_FIELD_CATALOG3 .
  ...
  LOOP AT GT_FIELDCAT3 INTO GS_FIELDCAT3.
    ...
    CASE GS_FIELDCAT3-FIELDNAME.
      ...
      WHEN 'SAKNR'.
        GS_FIELDCAT3-OUTPUTLEN = 20.

      WHEN 'DMBTR'.
        GS_FIELDCAT3-DO_SUM = 'X'.
        GS_FIELDCAT3-CFIELDNAME    = 'D_WAERS'.     " 통화 단위 참조
        GS_FIELDCAT3-COLTEXT       = '통화금액'.

      WHEN 'WRBTR'.
        GS_FIELDCAT3-DO_SUM = 'X'.
        GS_FIELDCAT3-COLTEXT    = '현지통화금액'.
        GS_FIELDCAT3-CFIELDNAME   = 'W_WAERS'.

      WHEN 'W_WAERS'.
        GS_FIELDCAT3-COLTEXT    = '현지통화코드'.   " 통화 단위 참조

      WHEN 'D_WAERS'.
        GS_FIELDCAT3-COLTEXT       = '통화코드'.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form MODIFY_DISPLAY_CELL_COL
*&---------------------------------------------------------------------*
FORM MODIFY_DISPLAY_CELL_COL
   CHANGING   C_EV_BELNR TYPE ZEA_FIT800-BELNR.

  LOOP AT GT_DATA INTO GS_DATA.

    IF GS_DATA-AUGBL EQ C_EV_BELNR.
...
ENDFORM.
*&---------------------------------------------------------------------*
*& Form GET_BASE_DATA_FOR_SCREE_0100
*&---------------------------------------------------------------------*
FORM GET_BASE_DATA_FOR_SCREE_0100 .
...
*    IF GS_TCURR-GDATU EQ '00000000'.
    ...
    ZEA_TCURR-FCURR = GS_TCURR-FCURR.
    ZEA_TCURR-TCURR = GS_TCURR-TCURR.
    ZEA_TCURR-GDATU = GS_TCURR-GDATU.
    ZEA_TCURR-UKURS = GS_TCURR-UKURS.

  ENDIF.

*--- 평균환율 읽어오기
  SELECT * FROM ZEA_TCURR INTO TABLE LT_TCURR
   WHERE FCURR EQ 'KRW'
    AND TCURR EQ 'USD'.

  LOOP AT LT_TCURR INTO LS_TCURR.
    LV_TOTAL = LV_TOTAL + LS_TCURR-UKURS.
    LS_TCURR-AVERAGE = LV_TOTAL / SY-TABIX. " <== 환율 평균
    MODIFY LT_TCURR FROM LS_TCURR TRANSPORTING AVERAGE.
  ENDLOOP.
  CLEAR LV_TOTAL.
  LV_TOTAL = LS_TCURR-AVERAGE.
